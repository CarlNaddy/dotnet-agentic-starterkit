# Live deployment runbook — carlnaddy-DotnetAgenticStarterkit
...
| | |
|---|---|
| URL | <https://carlnaddy-DotnetAgenticStarterkit.fly.dev/> |
| Fly app | `carlnaddy-DotnetAgenticStarterkit` |
| Region | `ams` (Amsterdam) |
| Database | `carlnaddy-DotnetAgenticStarterkit-db` — single-node Fly Postgres, same region |
| Admin login | `admin@DotnetAgenticStarterkit.local` + a generated password (see §3 — not reproduced here) |

## 2. Systems and tools involved

| System | Role |
|---|---|
| **This GitHub repo** (`CarlNaddy/DotnetAgenticStarterkit`) | Source of truth for code and for `fly.toml` (the app's runtime config: port, health checks, scaling, the `release_command`) |
| **Fly.io** | Hosts both the app (`carlnaddy-DotnetAgenticStarterkit`) and its database (`carlnaddy-DotnetAgenticStarterkit-db`) as separate Fly "apps," each made of one or more Machines |
...
|---|---|---|
| **App secrets** (`ConnectionStrings__Default`, `Seed__AdminPassword`) | Fly's own encrypted secret store, attached to the `carlnaddy-DotnetAgenticStarterkit` app | `fly secrets list -a carlnaddy-DotnetAgenticStarterkit` shows *names* only — Fly never returns values back out once set. To change one: `fly secrets set KEY=value -a carlnaddy-DotnetAgenticStarterkit` (triggers a new release) |
...
dotnet publish DotnetAgenticStarterkit.csproj -t:PublishContainer -c Release \
  ...
  -p:ContainerRepository=carlnaddy-DotnetAgenticStarterkit \
  -p:ContainerImageTag=<some-tag>
flyctl deploy --image registry.fly.io/carlnaddy-DotnetAgenticStarterkit:<some-tag> -a carlnaddy-DotnetAgenticStarterkit
```
...
```bash
flyctl status -a carlnaddy-DotnetAgenticStarterkit          # app + machine health
flyctl status -a carlnaddy-DotnetAgenticStarterkit-db       # database health
flyctl logs   -a carlnaddy-DotnetAgenticStarterkit          # stream app logs (Ctrl+C to stop)
flyctl logs   -a carlnaddy-DotnetAgenticStarterkit --no-tail  # just the recent buffer, no streaming
flyctl ssh console -a carlnaddy-DotnetAgenticStarterkit     # shell into a running app Machine
flyctl secrets list -a carlnaddy-DotnetAgenticStarterkit    # secret NAMES only, never values
flyctl machine restart <id> -a carlnaddy-DotnetAgenticStarterkit
```
