# Live deployment runbook — carlnaddy-dotnet-agentic-starterkit
...
| | |
|---|---|
| URL | <https://carlnaddy-dotnet-agentic-starterkit.fly.dev/> |
| Fly app | `carlnaddy-dotnet-agentic-starterkit` |
| Region | `ams` (Amsterdam) |
| Database | `carlnaddy-dotnet-agentic-starterkit-db` — single-node Fly Postgres, same region |
| Admin login | `admin@dotnet-agentic-starterkit.local` + a generated password (see §3 — not reproduced here) |
...
| **Fly.io** | Hosts both the app (`carlnaddy-dotnet-agentic-starterkit`) and its database (`carlnaddy-dotnet-agentic-starterkit-db`) as separate Fly "apps," each made of one or more Machines |
...
|---|---|---|
| **App secrets** (`ConnectionStrings__Default`, `Seed__AdminPassword`) | Fly's own encrypted secret store, attached to the `carlnaddy-dotnet-agentic-starterkit` app | `fly secrets list -a carlnaddy-dotnet-agentic-starterkit` shows *names* only — Fly never returns values back out once set. To change one: `fly secrets set KEY=value -a carlnaddy-dotnet-agentic-starterkit` (triggers a new release) |
...
dotnet publish dotnet-agentic-starterkit.csproj -t:PublishContainer -c Release \
  ...
  -p:ContainerRepository=carlnaddy-dotnet-agentic-starterkit \
  -p:ContainerImageTag=<some-tag>
flyctl deploy --image registry.fly.io/carlnaddy-dotnet-agentic-starterkit:<some-tag> -a carlnaddy-dotnet-agentic-starterkit
```
...
```bash
flyctl status -a carlnaddy-dotnet-agentic-starterkit          # app + machine health
flyctl status -a carlnaddy-dotnet-agentic-starterkit-db       # database health
flyctl logs   -a carlnaddy-dotnet-agentic-starterkit          # stream app logs (Ctrl+C to stop)
flyctl logs   -a carlnaddy-dotnet-agentic-starterkit --no-tail  # just the recent buffer, no streaming
flyctl ssh console -a carlnaddy-dotnet-agentic-starterkit     # shell into a running app Machine
flyctl secrets list -a carlnaddy-dotnet-agentic-starterkit    # secret NAMES only, never values
flyctl machine restart <id> -a carlnaddy-dotnet-agentic-starterkit
```
